...
#func damage(thrower):
#	print("i am name: [%s] obj: [%s]" % [self.name, str(self)])
#	print("damaged? [%s] [%s] [%s]" %  [str(thrower), str(get_node("/root/world/players/"+str(thrower))), str(self)])
##	if by_who == 
#	var winlabel = get_node("/root/world/winlabel")
#	if get_node("/root/world/players/"+str(thrower)) != self:
#
